...
If comparisons stopped immediately with `RuntimeError: Invalid device argument` and no completed preflight stages, pull the CUDA startup fix and rerun `comparisons`. CUDA is now explicitly initialized before resetting its memory counters. For the old uninstrumented startup failure, the workflow preserves the failed preflight in `preflight.failed.<id>/` and measures again, provided the configuration, dataset, and device match and no training artifacts exist. Other failures and completed run proofs are never replaced automatically. New preflight failures record the failing operation and traceback and print the underlying error in the command log. The completed field diagnostic does not need to be rerun.
